  ...
  return (
    ...
    >
      <div className="w-full md:w-[45%] flex justify-start md:justify-center mb-8 md:mb-0 relative">
        <div className="absolute top-8 md:top-1/2 -translate-y-1/2 left-[-15px] md:left-auto md:right-[-45px] w-6 h-6 rounded-full bg-cream border-2 border-ink/20 z-10 hidden md:block" style={{ borderColor: exp.color, boxShadow: `0 0 20px ${exp.color}55` }} />
        {/* Mobile node */}
        <div className="absolute top-2 left-[-24px] w-4 h-4 rounded-full bg-cream border-2 border-ink/20 md:hidden z-10" style={{ borderColor: exp.color, boxShadow: `0 0 10px ${exp.color}55` }} />
        ...
        <div className="pl-6 md:pl-0 w-full md:flex md:flex-col md:items-end md:pr-12 text-left md:text-right">
          ...
          <h3 className="text-2xl font-bold text-ink mb-1">{exp.role}</h3>
          <div className="flex items-center md:justify-end gap-2 text-ink/60 font-mono text-sm mb-2">
      ...
      <div className="w-full md:w-[45%] pl-6 md:pl-12">
        <div className="glass-card p-6 md:p-8 rounded-2xl border border-ink/10 relative overflow-hidden group hover:border-ink/20 transition-colors hover-lift" data-cursor-label="Details" data-cursor-magnetic>
          ...
          <ul className="space-y-4">
            {exp.bullets.map((bullet: string, i: number) => (
              <li key={i} className="flex gap-3 text-ink/70 text-sm leading-relaxed font-mono">
      ...
      <div className="container mx-auto px-6">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-cream border border-ink/10 rounded-sm mb-4">
            <Briefcase size={14} className="text-ink" />
            <span className="text-xs uppercase tracking-widest text-ink/70 font-mono">Experience Log</span>
          </div>
          <h2 className="text-4xl md:text-5xl font-display text-ink font-bold uppercase tracking-tight text-center">
            Professional <span className="ink-gradient-text">Experience</span>
          </h2>
        </motion.div>

        <div className="relative max-w-5xl mx-auto">
          {/* Cyber Center Line */}
          <div className="absolute left-[8px] md:left-1/2 top-0 bottom-0 w-[2px] -translate-x-1/2 bg-ink/10 rounded-full overflow-hidden">
            <motion.div 
